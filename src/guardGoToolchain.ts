...
export async function guardGoToolchain() {
  ...
  const goVersionResult = execSync('go version', { stdio: 'pipe' })
    .toString('utf-8')
    .trim();
  const [, versionMatch = ''] =
    /go\s+version\sgo((\d+\.?){1,3})/.exec(goVersionResult) ?? [];
  const minRequiredVersion = '1.16';
  const sortedSemver = [versionMatch, minRequiredVersion].sort();

  if (
    !versionMatch.startsWith(minRequiredVersion) &&
    sortedSemver[0] === versionMatch
  ) {
